...
export default function CopyInput({ text, handleOnCopy }: ICopyInput) {
  const onClickHandler = () => {
    handleOnCopy(text);
  };

  return (
    <div onClick={onClickHandler} className={styles.copy_input}>
      <span className={styles.copy_input__value}>{text}</span>
      {copyIcon()}
    </div>
  );
}
